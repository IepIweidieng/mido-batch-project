...
if "!現版:~,1!!現版:~-1!"=="""" (
 ...
 if "!現版:~,2!"=="0!現版:~1,1!" (set 現號=ver.!現版:~1,1!) else (set 現號=ver.!現版:~,2!)
 set "現號=ver.!現號!.!現版:~2!"
)
...
:升期
set 字址="https://docs.google.com/uc?authuser=0&id=0B1GI2ZfUddzgazM3S002bXI2dlU&export=download"
set 字檔="%temp%\MdS"
call:升下
set 版找=
set 字址=
...
if "!新版:~,1!!新版:~-1!"=="""" (
 ...
 if "!新版:~,2!"=="0!新版:~1,1!" (set 新號=ver.!新版:~1,1!) else (set 新號=ver.!新版:~,2!)
 set "新號=ver.!新號!.!新版:~2!"
)
...
if defined 字址 (
  ...
  <nul set/p=""&color 2f
  call:升單
  call:DT&>>"%~dpn0log.txt" echo;!_DT!　開始下載最新版（%新號%）
  color 27
  ...
  if !升選! equ 0 (
    ...
    if defined 檔找 (
      ...
      call:DT&>>"%~dpn0log.txt" echo;!_DT!　最新版（%新號%）下載完成
      "%~dp0%新名%.exe"
    ) else (
...
if %errorlevel%==5 (
>>"%~dpn0log.txt" echo;%MG% "%NT%"「Q」
color 27&set 升單錯=0&goto:eof
)
if %errorlevel%==4 (
>>"%~dpn0log.txt" echo;%MG% "%NT%"「X」
color 27&set 升選=1&goto:eof
)
if %errorlevel%==3 (
>>"%~dpn0log.txt" echo;%MG% "%NT%"「Z」
color 27&goto:eof
)
if %errorlevel%==2 (
...
call:DT
color 2f&cls
if "%1"=="0" echo;GOoD ByE with you！&>>"%~dpn0log.txt" echo;%_DT%　結束了更新程式
if "%1"=="00" echo;目前版本（%現號%）已是最新版！&>>"%~dpn0log.txt" echo;%_DT%　結束了更新程式
if "%1"=="1" >&2 echo;ＮＯ　ＩＮＴＥＲＮＥＴ！&>>"%~dpn0log.txt" echo;%_DT%　更新程式沒有網路而關閉
if "%1"=="2" >&2 echo;Ｄｏｗｎｌｏａｄｉｎｇ　Ｆａｉｌｅｄ！&>>"%~dpn0log.txt" echo;%_DT%　最新版（%新號%）下載失敗
if %10 gtr 60 >&2 echo;發生了嚴重的錯誤，程式必須關閉。&>>"%~dpn0log.txt" echo;%_DT%　更新程式遇到嚴重的錯誤而關閉
>>"%~dpn0log.txt" echo;~～~～~～~～~～~～~～~～~～~～~～~～~～~～~～~～~～~～~～~ 
endlocal&endlocal
