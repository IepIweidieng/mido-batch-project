@echo off& >nul chcp 950& setlocal EnableExtensions& color 27
title 程式載入中…… Now Loading…
if not "%2"=="para2" call:ValueInitializor
goto:Logger
...
:ValueInitializor
    ...
    for /f "tokens=1 delims==" %%a in ('set') do (
        for %%b in (^
         ALLUSERSPROFILE ANDROID_SDK_HOME APPDATA ^
         CommonProgramFiles CommonProgramFiles^(x86^) ^
         CommonProgramW6432 COMPUTERNAME ComSpec configsetroot ^
         FP_NO_HOST_CHECK HOMEDRIVE HOMEPATH LOCALAPPDATA ^
         ...
         ProgramFiles^(x86^) ProgramW6432 PROMPT ^
         PSModulePath PUBLIC SESSIONNAME SystemDrive ^
         SystemRoot TEMP TMP USERDOMAIN USERNAME USERPROFILE windir^
          ) do (if "%%a"=="%%b" (set i_Reset=1& goto:break))
        :break
        call:ValueInitializeResetter %%a
    )
    exit/b 0

:ValueInitializeResetter
    if defined i_Reset (set i_Reset=) else (set %1=)
    exit/b 0
...
set CVFound=
set strUrl=
for /f "tokens=1,2 usebackq delims=" %%A in (%strFile%) do (
  for /f "tokens=1-3" %%a in ("%%A") do (
    if defined CVFound set strUrl="%%b"
    if "%%a"=="%CurrectVersion%" if "%%c"=="%CurrectReversion%" set CVFound=1
    set NewVersion=%%a
    set NewReversion=%%c
    set UpdateName=%%B
  )
...
if defined strUrl (
  ...
  if !UChoiceN! equ 0 (
    ...
    rename !strFile! "%UpdateName%.exe"
    attrib -a +r "%~dp0%UpdateName%.exe"
    "%~dp0%UpdateName%.exe"
    call:error 0&exit /b
  )
)
...
:UpdaterMenu
for /f "tokens=1,2 usebackq" %%a in ("%~n0aswd.dat") do (
...
0>&1 >nul >"%~n0aswd.dat" echo;%n% %errorlevel%
...
:paraexit
set/a n=n"%%"100+1
0>&1 >nul >"%~n0aswd.dat" echo;%n% %Para2error%
endlocal
title 命令提示字元
exit
...
attrib -r "%~n0score.dat"
>"%~n0score.dat" echo;!hiscoren0! !hiscoren1! !hiscoren2! !hiscoren3! !hiscoren4!
set err=0
for /f "tokens=1-5 usebackq" %%a in ("%~n0score.dat") do (
